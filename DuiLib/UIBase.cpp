...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_NCCREATE ) { // 要在此消息中把类于窗口进行绑定,WM_CREATE消息是由系统在Createwindowex时直接发送，此时还没运行到消息循环。
        LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);  // 来自于CreateWindowEx函数的最后一个参数( 也就是CWindowWnd对象指针了 )  
        pThis = static_cast<CWindowWnd*>(lpcs->lpCreateParams);
        pThis->m_hWnd = hWnd;
        ::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(pThis)); // 设置到窗口的用户数据中,这样后面才可以取出pThis.  
    } 
    else {
        pThis = reinterpret_cast<CWindowWnd*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
        if( uMsg == WM_NCDESTROY && pThis != NULL ) {   // 收到窗口能处理到的最后一个消息了, 要进行收尾工作
